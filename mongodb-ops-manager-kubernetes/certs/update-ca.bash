...
kubectl -n ${namespace} create configmap ca-pem  --from-file="ca-pem=${PWD}/certs/ca.pem"

kubectl -n ${namespace} delete configmap ${omName}-ca > /dev/null 2>&1
kubectl -n ${namespace} create configmap ${omName}-ca --from-file="ca-pem=${PWD}/certs/ca.pem" --from-file="mms-ca.crt=${PWD}/certs/ca.pem"

"${PWD}/certs/make_OM_certs.bash" ${omName}

# delete all secrets except key-pair
kubectl -n ${namespace} get secrets | grep kubernetes | grep -v ca-key-pair | awk '{print $1}' | xargs -n 1 kubectl -n ${namespace} delete secret

kubectl -n ${namespace} delete secret         ${omName}-cert > /dev/null 2>&1
kubectl -n ${namespace} create secret generic ${omName}-cert \
