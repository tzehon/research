...
This app visually demonstrates WiredTiger cache behavior and memory sizing. The **[Observatory](https://github.com/mongodb)** view connects to any MongoDB 4.4+ cluster, displays seven real-time cache health metrics (dirty fill ratio, app thread eviction, disk reads, queued ops), and includes configurable load presets that deliberately stress the cache to show healthy vs. unhealthy states with color-coded thresholds and time-series charts. The **[Sizing Calculator](https://www.mongodb.com/docs/manual/reference/command/serverStatus/)** computes recommended `cacheSizeGB` and container memory limits based on working set size, connection counts, and deployment topology, with one-click auto-population from live metrics and exportable MCK YAML or Atlas tier recommendations.
